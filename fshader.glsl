...
uniform vec3 color; // Add this uniform for color
...
void main() {
    ...
    vec3 hslColor = vec3(color.x, color.y, color.z);
    vec3 rgbColor = hsl2rgb(hslColor);
	float brightness = dot(rgbColor, vec3(0.299, 0.587, 0.114));
	brightness = pow(brightness, 0.25); // Adjust the power value to control the transparency
    gl_FragColor = vec4(rgbColor * distanceInfo.x, brightness);
}
